    ...
    <div id="content">
    	<nav>
            <div class="nav nav-tabs border-0 text-center" id="nav-tab" role="tablist">
                <a class="nav-item nav-link sell-pending-nav-link active m-2 buying_p_l" style="width: 10rem;" id="nav-home-tab"
                   data-toggle="tab" href="#nav-home" role="tab"
                   aria-controls="nav-home" aria-selected="true">BUYING</a>
                <a class="nav-item nav-link m-2 sell-pending-nav-link buying_p_l_h" style="width: 10rem;" id="nav-profile-tab"
                   data-toggle="tab" href="#nav-home" role="tab"
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			var datatbl = $('.datatables').DataTable({
				processing: true,
				serverSide: true,
				ajax: "{{route('seller.buying.get_buy_products')}}",
			    "aaSorting": [],
				columns: [
			...
			$('.buying_p_l').click(function(){
		    	datatbl.ajax.url('{{ route("seller.buying.get_buy_products") }}').draw();
		    });
		    $('.buying_p_l_h').click(function(){
		    	datatbl.ajax.url('{{ route("seller.buying.get_buy_products_history") }}').draw(); 
	    	})
			// $(document).on('click','.received_p',function(){
			// 	Swal.fire({
			// 	  title: 'Are you sure?',
			// 	  text: "Product Re",
			// 	  icon: 'warning',
